...
VALID_ROUTES = ['8', '13']
VALID_STOPS = ['chalfonte', 'westview']
STOP_NAME_MAP = {
    "chalfonte": "Center Ave + Chalfonte Ave",
    "westview": "West View Plaza + Giant Eagle"
}

# Paired stops - each location has stop IDs for outbound and inbound directions
# Some stops use the same ID for both directions (API returns predictions based on bus direction)
# Others have different physical stops for each direction
PAIRED_STOPS = {
    "chalfonte": {"outbound": "1009", "inbound": "1016"},   # Different stops for each direction
    "westview": {"outbound": "619", "inbound": "619"}       # Same stop, different directions
}
...
def _empty_predictions_response(stop_name, route):
...
def _format_truetime_response(data, route=None, stop=None):
    ...
    # Use global STOP_NAME_MAP for consistency
    display_stop_name = STOP_NAME_MAP.get(selected_stop, STOP_NAME)
    ...
    try:
        ...
        if "error" in predictions:
            ...
            if any(msg.lower() in error_msg.lower() for msg in no_bus_messages):
                return _empty_predictions_response(display_stop_name, selected_route)
        ...
        for pred in pred_list:
            ...
            if selected_route and pred_route != selected_route:
                continue
...
def get_predictions_gtfsrt(route=None, stop=None):
    ...
    """
    try:
        stop = stop or "chalfonte"
        route = route or BUS_ROUTE
        stop_config = PAIRED_STOPS.get(stop, PAIRED_STOPS["chalfonte"])
        stop_name = STOP_NAME_MAP.get(stop, STOP_NAME)
...
def get_all_predictions():
    ...
    try:
        ...
        stop = request.args.get('stop', STOP_ID)
        ...
        if stop not in VALID_STOPS:
            return jsonify({
                "error": f"Invalid stop '{stop}'. Valid stops: {', '.join(VALID_STOPS)}"
            }), 400
        ...
        combined = {
            ...
            "schedule_period": period,
            "predictions": {
                "to_west_view": outbound_data["predictions"]["to_west_view"] if outbound_data else {"destination": DESTINATION_WEST_VIEW, "direction": "OUTBOUND", "arrivals": []},
                "to_downtown": inbound_data["predictions"]["to_downtown"] if inbound_data else {"destination": DESTINATION_DOWNTOWN, "direction": "INBOUND", "arrivals": []}
            }
        }
